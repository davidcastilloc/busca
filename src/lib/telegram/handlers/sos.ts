...
export async function startSos(
  ...
  try {
    ...
    console.error("Error en startSos:", err);
    await client.sendMessage(chatId, "❌ Error al iniciar el flujo SOS.");
  }
}
...
export async function handleSosState(
  ...
  try {
    ...
    console.error("Error general en handleSosState:", outerErr);
    await client.sendMessage(chatId, "❌ Ocurrió un error inesperado al procesar el estado SOS.");
    await clearSession(db, telegramId);
  }
}
