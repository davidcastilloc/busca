...
async function buscarPorCedula(DB: D1Database, cedula: string): Promise<ResultadoUnificado[]> {
  const [personas, reportes] = await Promise.all([
    DB.prepare("SELECT id, nombre, apellido, cedula, estado, sexo, edad, ubicacion_nombre, refugio, updated_at, foto_key, fuente FROM personas WHERE cedula = ? AND (fuente = 'web' OR fuente IS NULL)").bind(cedula).all(),
...
async function buscarPorNombre(DB: D1Database, query: string): Promise<ResultadoUnificado[]> {
  ...
  const [personas, reportes] = await Promise.all([
    DB.prepare(`SELECT id, nombre, apellido, cedula, estado, sexo, edad, ubicacion_nombre, refugio, updated_at, foto_key, fuente FROM personas WHERE (${pWhere.clause}) AND (fuente = 'web' OR fuente IS NULL) ORDER BY updated_at DESC LIMIT 50`)
...
async function buscarPorDescripcion(DB: D1Database, query: string): Promise<ResultadoUnificado[]> {
  ...
  const [personas, reportes] = await Promise.all([
    DB.prepare(`SELECT id, nombre, apellido, cedula, estado, sexo, edad, ubicacion_nombre, refugio, updated_at, foto_key, fuente FROM personas WHERE (${pCondiciones.join(" AND ")}) AND (fuente = 'web' OR fuente IS NULL) ORDER BY updated_at DESC LIMIT 50`)
      .bind(...pParams).all(),
